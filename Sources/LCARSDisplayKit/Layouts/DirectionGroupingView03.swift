...
open class DirectionGroupingView03: DirectionGroupingView {
    ...
    var innerRing01Shape: Crescent {
        let half = DPad.Arc.arc02.start + ((DPad.Arc.arc02.end - DPad.Arc.arc02.start) / 2)
        let innerArc = Arc(radius: firstRingInteriorRadius, startingDegree: DPad.Arc.arc01.start + 1.5, endingDegree: half)
        let outerArc = Arc(radius: firstRingExteriorRadius, startingDegree: DPad.Arc.arc01.start - 2.0, endingDegree: half)
    ...
    var innerRing08Shape: Crescent {
        let innerArc = Arc(radius: firstRingInteriorRadius, arc: .arc08)
        let outerArc = Arc(radius: firstRingExteriorRadius, arc: .arc08)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc)
    }
    
    var innerRing09Shape: Crescent {
        let innerArc = Arc(radius: firstRingInteriorRadius, arc: .arc09)
        let outerArc = Arc(radius: firstRingExteriorRadius, arc: .arc09)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc)
    }
    ...
    var innerRing14Shape: Crescent {
        let innerArc = Arc(radius: firstRingInteriorRadius, startingDegree: DPad.Arc.arc14.start, endingDegree: DPad.Arc.arc14.end - 1.0)
        let outerArc = Arc(radius: firstRingExteriorRadius, startingDegree: DPad.Arc.arc14.start, endingDegree: DPad.Arc.arc14.end - 1.0)
    ...
    var innerRing15Shape: Crescent {
        let innerArc = Arc(radius: firstRingInteriorRadius, startingDegree: DPad.Arc.arc15.start - 0.5, endingDegree: DPad.Arc.arc15.end + 0.5)
        let outerArc = Arc(radius: firstRingExteriorRadius, startingDegree: DPad.Arc.arc15.start - 0.5, endingDegree: DPad.Arc.arc15.end + 0.5)
    ...
    var innerRing16Shape: Crescent {
        let half = DPad.Arc.arc17.start + ((DPad.Arc.arc17.end - DPad.Arc.arc17.start) / 2)
        let innerArc = Arc(radius: firstRingInteriorRadius, startingDegree: DPad.Arc.arc16.start + 1.0, endingDegree: half)
        let outerArc = Arc(radius: firstRingExteriorRadius, startingDegree: DPad.Arc.arc16.start - 2.0, endingDegree: half)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc, extendExteriorStart: true, extendExteriorEnd: false)
    }
    
    var innerRing17Shape: Crescent {
        let half = DPad.Arc.arc17.start + ((DPad.Arc.arc17.end - DPad.Arc.arc17.start) / 2) + 1.5
        let innerArc = Arc(radius: firstRingInteriorRadius, startingDegree: half, endingDegree: DPad.Arc.arc19.end - 1.5)
        let outerArc = Arc(radius: firstRingExteriorRadius, startingDegree: half, endingDegree: DPad.Arc.arc19.end + 2.0)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc, extendExteriorStart: false, extendExteriorEnd: true)
    }
    ...
    var outerRing09Shape: Crescent {
        let innerArc = Arc(radius: secondRingInteriorRadius, arc: .arc09)
        let outerArc = Arc(radius: secondRingExteriorRadius, arc: .arc09)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc)
    }
    ...
    var outerRing19ExtendedShape: Crescent {
        let innerArc = Arc(radius: secondRingInteriorRadius, startingDegree: DPad.Arc.arc19.start, endingDegree: DPad.Arc.arc19.end + 2.0)
        let outerArc = Arc(radius: secondRingExtendedExteriorRadius, startingDegree: DPad.Arc.arc19.start, endingDegree: DPad.Arc.arc19.end + 2.0)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc, extendExteriorStart: false, extendExteriorEnd: true)
    }
    ...
    var outerRing20ExtendedShape: Crescent {
        ...
        let outerArc = Arc(radius: secondRingExtendedExteriorRadius, startingDegree: startDegree, endingDegree: endDegree)
        return Crescent(interiorArc: innerArc, exteriorArc: outerArc, extendExteriorStart: true, extendExteriorEnd: true)
    }
    ...
    var edge04Shape: EdgedCrescent {
        ...
        let arc = Arc(radius: Radius(thirdRingInteriorRadius), arc: .arc13)
    ...
    var edge05Shape: EdgedCrescent {
        ...
        let exteriorArc = Arc(radius: secondRingEdgeExteriorRadius, startingDegree: startDegree, endingDegree: endDegree)
